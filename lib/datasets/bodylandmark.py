...
def gen_landmark_map(image_w, image_h, landmark_in_pic, landmark_pos, R):
    ...
    for i in range(landmark_in_pic.shape[0]):
        if landmark_in_pic[i] == 0:
            ...
            #channel_map = gaussian_map(image_h, image_w, landmark_pos[i][0], landmark_pos[i][1], R)
...
class CustlrDataset(torch.utils.data.Dataset):#DeepFashionCAPDataset(torch.utils.data.Dataset):

    def __init__(self, df,cfg, mode,base_path):
        self.cfg = cfg
        self.df = df
        self.base_path = base_path
        self.to_tensor = transforms.ToTensor()  # pytorch使用c x h x w的格式转换
        self.rescale = Rescale(self.cfg.CONSTANT.RESIZE)
        self.rescale_largest_center = Rescale(self.cfg.CONSTANT.IMAGE_SIZE[0])
        self.rescalesquare = Rescale((self.cfg.CONSTANT.IMAGE_SIZE[0],self.cfg.CONSTANT.IMAGE_SIZE[1]))
        self.bbox_crop = BBoxCrop()
        self.center_crop = CenterCrop(self.cfg.CONSTANT.IMAGE_SIZE[0])
        self.random_crop = RandomCrop(self.cfg.CONSTANT.IMAGE_SIZE[0])
        ...
        self.mode = mode
        assert self.mode in ['RANDOM', 'CENTER', 'LARGESTCENTER', 'BBOXRESIZE', 'NOCROP']
    ...
    def plot_sample(self, i):
        ...
        #plt.show()
        #plt.savefig('sample/fullsize/lm4.jpg')
    ...
    def plot_landmark_map(self, i):
        ...
        plt.show()
        #plt.savefig('sample/fullsize/heatmap4.jpg')
    ...
    def __getitem__(self, i):
        ...
        if self.mode == 'RANDOM':
            ...
            image, landmark_pos = self.rescalesquare(image, landmark_pos)
        
        elif self.mode == 'NOCROP':
            image, landmark_pos = self.rescalesquare(image, landmark_pos)
        else:
        ...
        ret[self.cfg.CONSTANT.NAME_DIV8] = gen_landmark_map(int(image_w / 8), int(image_h / 8), landmark_in_pic, landmark_pos / 8, R / 8)
        ret[self.cfg.CONSTANT.NAME_DIV4] = gen_landmark_map(int(image_w / 4), int(image_h / 4), landmark_in_pic, landmark_pos / 4, R / 4)
        ret[self.cfg.CONSTANT.NAME_DIV2] = gen_landmark_map(int(image_w / 2), int(image_h / 2), landmark_in_pic, landmark_pos / 2, R / 2)
        ret[self.cfg.CONSTANT.NAME_NODIV] = gen_landmark_map(image_w, image_h, landmark_in_pic, landmark_pos, R)
        return ret
