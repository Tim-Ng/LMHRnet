...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, sample):
        ...
        lm_pos_reshaped = lm_pos_map.reshape(batch_size, self.cfg.DATASET.NUM_CLASSES, -1)
        # y是高上的坐标，x是宽上的坐标
    ...
    def cal_loss(self, sample, output):
            ...
            #    mask = sample['landmark_in_pic'].reshape(batch_size * self.cfg.DATASET.NUM_CLASSES, -1)
            #else:
            mask = sample['landmark_vis'].reshape(batch_size * self.cfg.DATASET.NUM_CLASSES, -1)
            mask = torch.cat([mask] * lm_size * lm_size, dim=1).float()

            map_sample = sample['landmark_map%d' % lm_size].reshape(batch_size * self.cfg.DATASET.NUM_CLASSES, -1)
            map_output = output['lm_pos_map'].reshape(batch_size * self.cfg.DATASET.NUM_CLASSES, -1)
            #training loss
